...
// STYLED COMPONENTS
import { HeaderProfile, ProfileInfoWrapper } from "../../components/profile.styles";
import { View } from "react-native";

export const DocumentTypeScreen = ({ navigation }) => {
  const user = useSelector((state) => state.authReducer.user),
    [documentType, setDocumentType] = useState(null);
    ...
    <SafeArea>
      ...
      <ProfileInfoWrapper>
        ...
        <ButtonSelect selected={documentType === "dni"} onPress={() => setDocumentType("dni")}>
          ...
          <Spacer variant="left" />
          <View>
            <Text variant="bold">Documento emitido por el país</Text>
        ...
        <ButtonSelect selected={documentType === "passport"} onPress={() => setDocumentType("passport")}>
          <PassportIcon />
          <Spacer variant="left" />
          <Text variant="bold">Pasaporte</Text>
        </ButtonSelect>
        ...
        <FlatList
          ...
          renderItem={({ item }) => {
            return <Text style={{ marginTop: 15 }}>{`\u2022 ${item.key}`}</Text>;
          }}
        />
        <Spacer variant="top" size={3} />
